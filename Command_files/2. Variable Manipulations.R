...
library(tidyverse)
theme_nice <- function() {
  theme_minimal(base_family = "Nanum Square") +
...
Analysis_data_noclean <- pick("Analysis_data/Analysis_data_noclean.dta")

### Aggregate welfare variables ------------------------------------------------

Analysis_data_noclean <- Analysis_data_noclean %>% mutate(
...
Figure1_cutpoint <- Analysis_data_noclean %>% 
  ...
  dplyr::select(v2x_polyarchy, v2xps_party) %>%
...
Analysis_data_noclean %>% drop_na(regime) %>% 
  ...
  select(v2x_polyarchy) %>% count() %>% rename(demo_under = n) -> demo_under

Analysis_data_noclean %>% drop_na(regime) %>% 
  dplyr::filter(regime=="Autocracies" & v2x_polyarchy > 0.5) %>% 
  select(v2x_polyarchy) %>% count() %>% rename(auto_upper = n) -> auto_upper
...
Analysis_allregime <- Analysis_allregime %>% 
  select(
    country_name, COWcode, year, e_lexical_index, v2x_polyarchy, regime,
...
class_year_auto <- Analysis_auto %>% 
  select(class, year, COWcode, country_name) %>%
  dplyr::filter(class %in% class_labels) %>% 
  group_by(class, year) %>% summarize(freq = n())

#### Path Plot -----------------------------------------------------------------
class_year_auto %>% 
  mutate(
    label = if_else(year == max(year), 
                    as.character(class), NA_character_)) %>%
  ggplot(aes(x = year, y = freq, color = class)) + 
  geom_path() +
  scale_x_continuous(
    limits = c(1910, 2020),
...
ggsave("Documents/2_Manuscript/2_Figures/Figure2.pdf", 
...
ggsave("Documents/2_Manuscript/2_Figures/Appendix/Appendix_Figure2A.pdf", 
       width = 8.5, height = 5, dpi = "retina")

### Yearly Trends of Essential Class Coalitions by Regimes ---------------------
#### Yearly frequencies --------------------------------------------------------
class_year <- Analysis_allregime %>% 
  select(class, year, COWcode, country_name, regime) %>%
  dplyr::filter(class %in% class_labels) %>% 
  group_by(class, regime, year) %>% summarize(freq = n())
...
class_summary_auto <- Aanalysis_auto %>% 
  select(class, totalunivers, v2regsupgroupssize, v2dlunivl, year) %>%
  dplyr::filter(class %in% class_labels & 
                  year > 1916) %>%
  group_by(class) %>% 
...
class_summary_demo <- Aanalysis_demo %>% 
  select(class, totalunivers, v2regsupgroupssize, v2dlunivl, year) %>%
  dplyr::filter(class %in% class_labels & 
                  year > 1916) %>%
  group_by(class) %>% 
...
other_summary_auto <- Aanalysis_auto %>% 
  select(class, totalunivers, v2regsupgroupssize, v2dlunivl, year) %>%
  dplyr::filter(class %in% others_labels & 
                  year > 1916) %>%
  group_by(class) %>% 
...
other_summary_demo <- Aanalysis_demo %>% 
  select(class, totalunivers, v2regsupgroupssize, v2dlunivl, year) %>%
  dplyr::filter(class %in% others_labels & 
                  year > 1916) %>%
  group_by(class) %>% 
...
#### Figure 3 (Autocracy and Essential class coalition) ------------------------
...
Aanalysis_auto %>% 
  ...
  geom_text(
    data = class_summary_auto %>% 
      dplyr::filter(class %in% "Party elites"),
    map = aes(x = 30, y = 0.05), size = 3,
    label = paste0("Mean U.I. = ", round(class_summary_auto[1, 2], 1))) +
  geom_vline(
    data= class_summary_auto %>% 
  ...
  geom_text(
    data= class_summary_auto %>% 
      dplyr::filter(class %in% "The military"), 
    map = aes(x = 30, y = 0.05), size = 3,
    label = paste0("Mean U.I. = ", round(class_summary_auto[2, 2], 1))) + 
  geom_vline(
    data= class_summary_auto %>% 
  ...
  geom_text(
    data= class_summary_auto %>% 
      dplyr::filter(class %in% "Urban Working"), 
    map = aes(x = 30, y = 0.05), size = 3,
    label = paste0("Mean U.I. = ",round(class_summary_auto[3, 2], 1))) + 
  geom_vline(
    data= class_summary_auto %>% 
  ...
  geom_text(
    data= class_summary_auto %>% 
      dplyr::filter(class %in% "Urban Middle"), 
    map = aes(x = 30, y = 0.05), size = 3,
    label = paste0("Mean U.I. = ", round(class_summary_auto[4, 2], 1))) + 
...
ggsave("Documents/2_Manuscript/2_Figures/Figure3.pdf", 
       width = 8.5, height = 5, dpi = "retina")
