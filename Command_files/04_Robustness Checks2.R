...
modelsummary::modelsummary(list(
  "Model 1" = fixed1_wo_rep1, 
  "Model 2" = fixed1_wo_civil1, 
  "Model 3" = cso_model_fixed1, 
  "Model 4" = cso_model_fixed3, 
  "Model 5" = cso_model_fixed5, 
  "Model 6" = full_model_fixed1, 
  "Model 7" = fixed1_wo_rep2, 
  "Model 8" = fixed1_wo_civil2, 
  "Model 9" = cso_model_fixed2, 
  "Model 10" = cso_model_fixed4, 
  "Model 11" = cso_model_fixed6, 
  "Model 12" = full_model_fixed2), 
  stars = TRUE,
  coef_map = c("l_mass_coalition" = "Mass-Based Coalition (vs. Elite-Based)",
               "l_massparty" = "Mass Party Organization Index",
  ...
  add_rows = rows,
  coef_omit = "as.factor",
  gof_omit = 'DF|Deviance|R2|adjR2', output = 'gt')
...
## Linear Interaction Assumption Test -------------------------------------------------------------

library(interflex)
sample <- baseline |> ungroup() |> 
  dplyr::filter(brm_regime %in% "Autocracies (BRM)") |> 
  dplyr::select(universality, mass_coalition, l_mass_coalition, l_massparty,
                l_gdppc, l_gdpgrth, l_lnresource,
                l_civilwar, l_repression, l_CSOconsult, l_CSOpart,
                l_hereditaryindex, l_militaryindex, l_partyindex, l_personindex,
                COWcode, year, vdem_regime) |> 
  mutate(lngpppc = log(l_gdppc+1)) |> as.data.frame()

glimpse(sample)
table(sample$l_mass_coalition)
LIE_check <- 
  interflex(Y = "universality", D = "l_mass_coalition", X = "l_massparty", 
            Z = c("l_gdpgrth", "l_gdppc", "l_lnresource", "l_civilwar",
                  "l_repression", "l_CSOconsult", "l_CSOpart",
                  "l_hereditaryindex", "l_militaryindex", "l_partyindex", "l_personindex"),
            data = sample, estimator = "binning", 
            FE = c("COWcode","year"),
            #cl = c("COWcode"),
            #cl = NULL, 
            #vartype = "simu",
            ...
            cores = 4,
            nbins = 2,
            Ylabel = "Welfare Universalism",
            Dlabel = "Mass Coalition",
            Xlabel = "Mass Party Organization Index",
            #ylab = "Marginal Effect of Mass Coalition on Welfare Universalism",
            #xlab = "Mass Party Organization Index",
            #Ylabel = "Marginal Effect of Mass Coalition on Welfare Universalism",
            #nboots = 200,
            #xlim = c(0, 10),
            #ylim = c(-9, 15), 
            theme.bw = TRUE)
plot(LIE_check)
getwd()
ggsave("Documents/2_Manuscript/2_Figures/fig_LIE_brm.pdf", 
       width = 8.5, height = 6, dpi = "1600")
sample |> 
  drop_na(l_mass_coalition) |> 
  mutate(l_mass_coalition = factor(l_mass_coalition,
                                   levels = c(
                                     "Mass-based", "Institution-based"),
                                   labels = c("Mass-based", "Elite-based"))) |> 
  ggplot(aes(l_massparty, fill = l_mass_coalition)) +
  labs(x = NULL, y = NULL) +
  geom_density(aes(y = ..scaled..), alpha = 0.2) + facet_wrap(~l_mass_coalition) +
  scale_fill_manual(values = c(futurevisions::futurevisions("mars")[3],
                               futurevisions::futurevisions("mars")[1])) +
  scale_y_continuous(labels = scales::percent_format()) +
  theme(legend.title = element_blank())
ggsave("Documents/2_Manuscript/2_Figures/fig_DistX_spsa.pdf", 
